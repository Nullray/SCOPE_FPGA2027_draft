...
FireSim supplies cycle-exact, FPGA-accelerated full-system simulation and
configurable scale-out models~\cite{karandikar2018firesim}. FASED separates a
target DRAM model from its FPGA host implementation and makes
...
\subsection{Modular Full-System Simulation}

SimBricks links simulators for machines, networks, and devices through explicit
component interfaces and synchronized communication
channels~\cite{li2022simbricks}. It can integrate existing NIC and switch RTL
models, making modular simulation a strong option when a suitable model exists.
\sys addresses the complementary case in which a processor prototype must run
an unmodified native driver against a physical peripheral. This trades the
controllability of a device model for execution by real controller hardware and
firmware, while giving up cycle-exact timing across the mediated boundary.
...
fabric resources to hosts; PCIe switches can route those functions between
hosts and devices. Their control plane operates on real PCIe functions
