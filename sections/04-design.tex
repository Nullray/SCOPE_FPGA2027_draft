...
The design places a configurable device boundary between the native driver and
the physical backend. Topology and configuration establish that boundary;
semantic mediation and DMA realization preserve it during I/O. The following
mechanisms enforce the three coherence invariants in
Section~\ref{sec:overview} while retaining a generic FPGA interface.

\begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{figures/coherence_protocols.pdf}
    \caption{The three ordering contracts that preserve one driver-visible
    device across distributed state. In each panel, downward arrows denote the
    required happens-before order; the final green event may become visible only
    after the preceding state transition.}
    \label{fig:coherence}
    \Description{Three sequences show configuration and route update before
    write completion, queue stabilization and address translation before a
    physical doorbell, and data visibility before completion and interrupt.}
\end{figure*}
...
The selectable view is therefore constrained by both the semantic adapter and
the physical backend. Changing an identity field alone cannot make a backend
implement a different device protocol.
...
This boundary determines which operations require virtualization. State that
contains DUT addresses or affects virtual configuration, ownership, or interrupt
visibility requires mediation. Operations whose semantics remain valid at the
backend can retain physical execution after this adaptation. Bypassing a
required translation would violate the device contract even if it reduced
control latency.
...
The evaluation therefore examines native-driver compatibility and control/data
cost separately from the unmeasured capacity-scaling hypothesis.
